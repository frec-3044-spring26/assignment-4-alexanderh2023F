...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) + 
  ...
  labs(x = 'Year', y = 'Ice-Off Day of Year', 
       title = 'Ice-Off Days in Lake Sunapee Through the Years')
```
...
I expect the sign to be negative, as a trend can be seen from the visualization where as the year increases (approaches the present), the ice off day decreases (happens earlier).
...
The direction of the slope is negative, matching what I expected earlier in question 2.
...
The R2 for the regression is 0.2055. According to this value, the `year` and `ice_off_doy` variables are not strongly related to each other.
...
```{r}
library(modelr)
sunapee <- add_predictions(data = sunapee, model = fit)
sunapee <- add_residuals(data = sunapee, model = fit)
```
...
The residuals do not seem to meet the assumptions of the linear regression, as the data is skewed to the left, and not normally distributed.
